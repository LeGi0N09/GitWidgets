    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
        ...
        <defs>
            ...
            <linearGradient id="grad-${uid}" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%"   stop-color="${data.colorFrom}"/>
                <stop offset="100%" stop-color="${data.colorTo}"/>
            </linearGradient>
            <!-- Glass shimmer on pill -->
            <linearGradient id="pill-sheen-${uid}" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%"   stop-color="#ffffff" stop-opacity="0.18"/>
                <stop offset="60%"  stop-color="#ffffff" stop-opacity="0.04"/>
                <stop offset="100%" stop-color="#ffffff" stop-opacity="0.0"/>
            </linearGradient>
            <!-- Outer glow filter -->
            <filter id="pill-shadow-${uid}" x="-8%" y="-15%" width="116%" height="130%">
                <feDropShadow dx="0" dy="${Math.round(2 * scale)}" stdDeviation="${Math.round(3 * scale)}"
                    flood-color="${data.colorTo}" flood-opacity="0.25"/>
            </filter>
        </defs>

        <!-- Pill base -->
        <rect width="${tagWidth}" height="${TAG_HEIGHT}" rx="${rx}"
            fill="${theme.background}" filter="url(#pill-shadow-${uid})"/>

        <!-- Pill border with accent color -->
        <rect width="${tagWidth}" height="${TAG_HEIGHT}" rx="${rx}"
            fill="none" stroke="${data.colorTo}" stroke-width="1.5" stroke-opacity="0.5"/>

        <!-- Glass shimmer -->
        <rect width="${tagWidth}" height="${TAG_HEIGHT}" rx="${rx}"
            fill="url(#pill-sheen-${uid})"/>
        ...
        <!-- Icon circle glass sheen -->
        <rect x="${H_PAD - 2 * scale}" y="${(TAG_HEIGHT - ICON_BOX) / 2}"
            width="${ICON_BOX}" height="${ICON_BOX / 2}" rx="${ICON_BOX / 2}"
        ...
        <text x="${H_PAD + ICON_BOX + ICON_TEXT_GAP}" y="${TAG_HEIGHT / 2 + FONT_SIZE * 0.35}"
            ...
            font-weight="600" fill="${theme.title}">
            ${label}
        </text>
    </svg>`
}
